...
export default function EnrolledCoursesSection() {
      ...
      try {
        ...
        const statsEntries = await Promise.all(
          courseIds.map(async (courseId) => {
            ...
            const total = (lessonsRes.data?.data?.total ??
              lessonsRes.data?.data?.items?.length ??
              lessonsRes.data?.data?.length ??
              0) as number;
  ...
  return (
    <Stack
      spacing={2}
      sx={{ maxWidth: 1080, mx: "auto", px: { xs: 2, md: 4 } }}
    >
      <Box
        sx={{
          display: "flex",
            ...
            if (nowOpen && !lessonsState) {
              ...
              try {
                ...
                try {
                  ...
                  const first = await axiosClient.get<ApiResponse<any>>(
                  ...
                  const firstList: Array<{ id: string; title?: string }> =
                    first.data?.data?.items ?? first.data?.data ?? [];
                  const totalLessons: number =
                    first.data?.data?.total ?? firstList.length;
                  acc.push(...firstList);
                  const totalPages = Math.ceil(totalLessons / pageSize);
                  for (let p = 2; p <= totalPages; p++) {
                    const r = await axiosClient.get<ApiResponse<any>>(
                      ROUTES_API_LESSON.PREVIEW,
                      {
                        params: {
                          courseId: enrollment.courseId,
